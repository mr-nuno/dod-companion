...
public sealed class RulesTokenProvider(
    ...
    ILogger<RulesTokenProvider> logger) : IRulesTokenProvider
{
    ...
    private async Task<string> FetchTokenAsync(CancellationToken ct)
    {
        ...
        _expiresAt = DateTimeOffset.UtcNow.AddSeconds(payload.ExpiresIn) - ExpiryMargin;

        logger.LogInformation("Acquired Rules API token, valid for {ExpiresIn}s", payload.ExpiresIn);
        return _token;
    }
